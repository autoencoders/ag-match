#!/usr/bin/env sh
# Refresh the `package` branch: the contents of src/namematch at the root, nothing else.
# Apps vendor it with:
#   git subtree add --prefix=<path> <repository-url> package --squash
# Run from any checkout after the source changes land, e.g. ./scripts/publish-package-branch.sh main
set -eu
src="${1:-HEAD}"
sha="$(git subtree split --prefix=src/namematch "$src")"
